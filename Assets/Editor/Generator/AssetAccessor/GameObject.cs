  ...
  public static class GameObjectAccessor
  {
    ...
    private static void GenerateGameObjectFiles(string outputDir)
    {
      ...
      foreach (var scene in scenes)
      {
        ...
        // Collect and add using directives from custom script source files
        var customScriptUsings = CollectCustomScriptUsings(gameObjects);
        foreach (var usingDirective in customScriptUsings)
        {
          _ = sb.AppendLine(usingDirective);
        }
        ...
        var generatedClassNames = new Dictionary<string, string>();
        var rootClassPath = $"AssetAccessor.Scene.{sceneEnumName}.GameObject";
        foreach (var go in rootGameObjects)
        {
          GenerateGameObjectClass(sb, go, gameObjects, 8, usedNames, generatedClassNames, rootClassPath);
        }
    ...
    private static void GenerateGameObjectClass(StringBuilder sb, GameObjectInfo go, List<GameObjectInfo> allGameObjects, int indent, HashSet<string> usedNames, Dictionary<string, string> generatedClassNames, string parentClassPath = "")
    {
      ...
      generatedClassNames[go.Id] = className;

      // Full path used to generate unambiguous global:: references inside nested Component.Script
      var currentClassPath = string.IsNullOrEmpty(parentClassPath) ? className : $"{parentClassPath}.{className}";
      ...
      if (!string.IsNullOrEmpty(go.ParentId))
      {
        // Child GameObject: find via parent's transform
        if (!generatedClassNames.TryGetValue(go.ParentId, out var parentClassName))
        {
          throw new InvalidOperationException(
            $"[AssetAccessorGenerator.GameObjectAccessor] Parent class name for ID '{go.ParentId}' (parent of '{go.Name}') was not recorded. This indicates a processing order bug.");
        }

        var nameForFind = EscapeStringLiteral(go.Name);
        var nameForMessage = EscapeInterpolatedString(go.Name);
        _ = sb.AppendLine($"{indentStr}    var parent = {parentClassName}.Get();");
        _ = sb.AppendLine($"{indentStr}    var child = parent.transform.Find(\"{nameForFind}\");");
        _ = sb.AppendLine($"{indentStr}    if (child == null)");
        _ = sb.AppendLine($"{indentStr}    {{");
        _ = sb.AppendLine($"{indentStr}      throw new System.Exception($\"[AssetAccessor.GameObject] Child GameObject '{nameForMessage}' not found under parent\");");
        _ = sb.AppendLine($"{indentStr}    }}");
        _ = sb.AppendLine($"{indentStr}    return child.gameObject;");
      }
      else
      {
        // Root GameObject or PrefabInstance root: find via cache
        var nameForGet = EscapeStringLiteral(go.Name);
        var nameForMessage = EscapeInterpolatedString(go.Name);
        _ = sb.AppendLine($"{indentStr}    return Scene.GameObjectCacheController.GetGameObject(\"{nameForGet}\")");
        _ = sb.AppendLine($"{indentStr}      ?? throw new System.Exception($\"[AssetAccessor.GameObject] GameObject '{nameForMessage}' not found or scene not loaded\");");
      }
      ...
      {
        var scriptAlias = GetScriptAlias(customScriptWithMembers.TypeFullName);
        foreach (var member in customScriptWithMembers.CustomScriptMembers)
        {
          ...
          if (member.Type is MemberType.Property or MemberType.Field)
          {
            ...
            if (member.IsStatic)
            {
              // Static members are accessed directly on the type alias, no instance needed
              _ = sb.AppendLine($"{indentStr}    get {{ return {scriptAlias}.{member.Name}; }}");
              if (member.HasSetter)
              {
                _ = sb.AppendLine($"{indentStr}    set {{ {scriptAlias}.{member.Name} = value; }}");
              }
            }
            else
            {
          ...
          else if (member.Type == MemberType.Method)
          {
            ...
            _ = member.IsStatic
              ? sb.AppendLine($"{indentStr}    {returnPrefix}{scriptAlias}.{member.Name}({arguments});")
              : sb.AppendLine($"{indentStr}    {returnPrefix}Component.Script.Get{customScriptWithMembers.SafeName}().{member.Name}({arguments});");
      ...
      if (go.Components.Count > 0)
      {
        ...
        foreach (var component in go.Components.Where(c => !c.IsCustomScript))
        {
          ...
          _ = sb.AppendLine($"{indentStr}      {{");
          _ = sb.AppendLine($"{indentStr}        var go = {currentClassPath}.Get();");
      ...
      if (children.Count > 0)
      {
        ...
        {
          GenerateGameObjectClass(sb, child, allGameObjects, indent + 2, childUsedNames, generatedClassNames, currentClassPath);
        }
      }
    ...
    private static List<GameObjectInfo> ParseSceneGameObjects(string scenePath)
    {
      ...
      foreach (var block in transformBlocks)
      {
        ...
        {
          if (!transformToGameObject.TryGetValue(parentTransformId, out string parentGameObjectId))
          {
            // Parent is a stripped transform from a PrefabInstance — map to its virtual GO ID
            if (prefabTransformToPrefabId.TryGetValue(parentTransformId, out var parentPrefabId))
            {
              parentGameObjectId = $"prefab_{parentPrefabId}";
            }
          }

          if (parentGameObjectId != null)
          {
      ...
      foreach (var go in gameObjects.Where(g => g.IsPrefabInstance))
      {
        ...
        if (!string.IsNullOrEmpty(parentTransformId) && parentTransformId != "0")
        {
          ...
          else if (prefabTransformToPrefabId.TryGetValue(parentTransformId, out var parentPrefabId))
          {
            // Parent is a stripped transform from another PrefabInstance
            go.ParentId = $"prefab_{parentPrefabId}";
          }
        }
      }
    ...
    private static List<string> CollectCustomScriptUsings(List<GameObjectInfo> gameObjects)
    {
      var namespaces = new HashSet<string> { "UnityEngine" }; // already emitted
      var aliasDirectives = new HashSet<string>();             // "using X = Y;" directives
      var usingPattern = new Regex(@"^\s*using\s+(?!static\s)(\w[\w.]*)\s*;", RegexOptions.Multiline);
      var aliasPattern = new Regex(@"^\s*using\s+(\w+)\s*=\s*([\w.]+)\s*;", RegexOptions.Multiline);
      ...
      foreach (var component in gameObjects.SelectMany(go => go.Components).Where(c => c.IsCustomScript))
      {
        ...
        foreach (Match match in usingPattern.Matches(scriptContent))
        {
          ...
          // Exclude editor-only namespaces — they must not appear in generated runtime files
          if (!IsEditorOnlyNamespace(ns))
          {
            _ = namespaces.Add(ns);
          }
        }

        // Collect namespace/type alias directives (using Alias = Target)
        foreach (Match match in aliasPattern.Matches(scriptContent))
        {
          var aliasTarget = match.Groups[2].Value.Trim();
          if (!IsEditorOnlyNamespace(aliasTarget))
          {
            _ = aliasDirectives.Add(match.Value.Trim());
          }
        }
      }

      _ = namespaces.Remove("UnityEngine");
      var result = namespaces.OrderBy(ns => ns).Select(ns => $"using {ns};").ToList();
      result.AddRange(aliasDirectives.OrderBy(a => a));
      return result;
    }

    private static bool IsEditorOnlyNamespace(string ns)
    {
      return ns == "UnityEditor" || ns.StartsWith("UnityEditor.");
    }
    ...
    private static string ResolveTypeName(string typeName, string scriptContent, string scriptTypeFullName, string scriptNamespace, HashSet<string> fileLevelTypes)
    {
      ...
      // If the type name refers to the enclosing class itself, return its alias directly
      var enclosingClassName = scriptTypeFullName.Split('.').Last();
      if (typeName == enclosingClassName)
      {
        return GetScriptAlias(scriptTypeFullName);
      }
    ...
    private static void ParseCustomScriptMembers(ComponentInfo componentInfo)
    {
      ...
      var propertyPattern = new Regex(
        @"^\s*public\s+(static\s+)?(?:(?:virtual|override|abstract)\s+)?([\w.<>\[\],\(\)\s]+)\s+(\w+)\s*\{([^}]*)\}",
        RegexOptions.Multiline
      );
      var propertyMatches = propertyPattern.Matches(scriptContent);
      foreach (Match match in propertyMatches)
      {
        var isStatic = match.Groups[1].Success;
        var returnType = match.Groups[2].Value.Trim();
        var memberName = match.Groups[3].Value.Trim();
        var propertyBody = match.Groups[4].Value.Trim();
      ...
      // Parse public readonly fields (including static readonly)
      ...
      {
        var isStatic = match.Groups[1].Success;
        var fieldType = match.Groups[2].Value.Trim();
      ...
      var methodPattern = new Regex(
        @"^\s*public\s+(static\s+)?(?:(?:virtual|override|abstract|async)\s+)?([\w.<>\[\]]+)\s+(\w+)\s*\(([^)]*)\)",
        RegexOptions.Multiline
      );
      ...
      foreach (Match match in methodMatches)
      {
        ...
        var returnType = match.Groups[2].Value.Trim();
        var memberName = match.Groups[3].Value.Trim();
        var parametersStr = match.Groups[4].Value.Trim();
